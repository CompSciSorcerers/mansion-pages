...
import MansionDeathBarrier from './MansionDeathBarrier.js';
...
class BarrierManager extends GameObject {
    constructor(data = null, gameEnv = null) {
        ...
        this.barriers = [];
        this.barrierSpawnTimes = {}; // Track spawn time for each threshold
        this.lastBossHealthPercent = 100;
        this.BARRIER_DURATION_MS = 10000; // 10 seconds
    ...
    update() {
        ...
        // Check if fade is complete before spawning new barriers
        const fadeComplete = typeof window === 'undefined' || window.__battleRoomFadeComplete !== false;
        
        // Check for threshold crossings (75%, 50%, 25%)
        for (const threshold of [75, 50, 25]) {
            // Only trigger once per threshold, and only spawn if fade is complete
            if (!this.triggeredThresholds[threshold] && currentHealthPercent <= threshold && fadeComplete) {
                this.triggeredThresholds[threshold] = true;
                this.spawnBarriersForThreshold(threshold);
            }
        }

        // Check if barriers should be destroyed after 10 seconds
        const now = Date.now();
        for (const threshold of Object.keys(this.barrierSpawnTimes)) {
            const spawnTime = this.barrierSpawnTimes[threshold];
            if (now - spawnTime > this.BARRIER_DURATION_MS) {
                this.destroyBarriersForThreshold(threshold);
                delete this.barrierSpawnTimes[threshold];
            }
        }
    ...
    spawnBarriersForThreshold(threshold) {
        ...
        // Record spawn time for this threshold
        this.barrierSpawnTimes[threshold] = Date.now();
        ...
        for (let i = 0; i < config.count; i++) {
            ...
            const barrier = new MansionDeathBarrier({
                x: pos.x - config.size / 2,
                y: pos.y - config.size / 2,
                width: config.size,
                height: config.size,
                pixelSize: Math.floor(config.size / 4) // 4 pixels per dimension
            }, this.gameEnv);
            
            barrier.visible = true;
            barrier.threshold = threshold; // Tag barrier with threshold for destruction
    ...
    destroyBarriersForThreshold(threshold) {
        // Find all barriers with this threshold
        const barriersToRemove = this.barriers.filter(b => b.threshold === threshold);
        
        for (const barrier of barriersToRemove) {
            // Remove from barriers array
            const idx = this.barriers.indexOf(barrier);
            if (idx > -1) this.barriers.splice(idx, 1);
            
            // Remove from game objects
            const gameObjIdx = this.gameEnv.gameObjects.indexOf(barrier);
            if (gameObjIdx > -1) this.gameEnv.gameObjects.splice(gameObjIdx, 1);
