#!/usr/bin/env python3
"""Run make and send the resulting Jekyll log to Flask Gemini for analysis."""
...
def wait_for_log(path: Path, timeout: int = 30) -> bool:
    start = time.time()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run make and analyze /tmp/jekyll4500.log with Flask Gemini."
    )
    parser.add_argument(
        "--cmd",
        default=DEFAULT_CMD,
    ...
    if not wait_for_log(Path(args.log), timeout=30):
